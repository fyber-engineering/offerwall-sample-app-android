...
def unlock_item_status_visible?
  query('DialogTitle', :text).first == 'Response From SponsorPay Unlock Server'
end

def wait_for_unlock_item_status
  wait_for(10) {
    sleep 1
    unlock_item_status_visible?
  }
end
