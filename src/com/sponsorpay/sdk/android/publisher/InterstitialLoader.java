 ...
 * the {@link InterstitialActivity} if and ad is returned.
 * </p>
 * <p>
 * Shows a progress dialog on top of the calling activity while it contacts the SponsorPay’s servers to determine
 * whether there’s an interstitial available.
 * </p>
 * <p>
 * If an ad is available, it will continue showing the loading progress dialog while the ad contents are loaded in the
 * background. The entire process must happen in less than {@value #LOADING_TIMEOUT_SECONDS_DEFAULT} seconds (or a
 * custom amount of time that can be configured, see {@link #setLoadingTimeoutSecs(int)}). Otherwise a timeout error
 ...
 * be shown on the screen (by showing the {@link InterstitialActivity}).
 * </p>
 * <p>
 * If no ad is available for this request, the loading progress dialog will disappear too and the process will be
 * completed.
 * </p>
 * <p>
 * An {@link InterstitialLoadingStatusListener} can be defined which will be notified of interesting events in the life
 * of the interstitial.
 * </p>
 */
public class InterstitialLoader {
	/**
	 * Interface to be implemented by parties interested in being notified of interesting events in the life of the
	 * interstitial.
	 */
	public interface InterstitialLoadingStatusListener {
		/**
		 * Invoked when SponsorPay’s backend has returned an ad. The {@link InterstitialActivity} is about to be shown.
		 */
		void onWillShowInterstitial();

		/**
		 * Invoked when a response has been received from SponsorPay’s backend indicating that no interstitial ad is
		 * available for this particular request.
		 */
		void onNoInterstitialAvailable();

		/**
		 * Invoked when the interstitial ad could not be requested or loaded due to an error. The device might not have
		 * a working connection to the Internet.
		 */
		void onInterstitialRequestError();

		/**
		 * Invoked when the interstitial ad request or the loading of the interstitial contents could not be performed
		 * within the amount of time defined by the loading timeout value.
		 */
	...
	/**
	 * <p>
	 * Requests and loads an interstitial ad in the background. Will call the {@link InterstitialLoadingStatusListener}
	 * registered in the {@link InterstitialLoader} in the same thread which triggered the request / loading process.
	 * Uses the Android {@link AsyncTask} mechanism.
	 * </p>
	 * <p>
	...
	public class InterstitialLoadingAsyncTask extends AsyncTask<String, Integer, InterstitialLoadingResults> {
		...
		/**
		 * Override from {@link AsyncTask}. Will run in the thread that triggered the load / execute process (calling
	...
	/**
	 * The activity which created this InterstitialLoader instance. Used to launch the {@link InterstitialActivity} and
	...
	/**
	 * Can be set to the absolute URL of an image to use as background graphic for the interstitial. Must include the
	 * protocol scheme (http:// or https://) at the beginning of the URL. Leave it null for no custom background.
	 * 
	 * @param backgroundUrl
	 */
	...
	/**
	 * Used to specify the behavior of the interstitial once the user clicks on the presented ad and is redirected
	 * outside the host publisher app. The default behavior is to close the interstitial and let the user go back to the
	 * activity that called the interstitial when they come back to the app. If you want the interstitial not to close
	 * until the user does it explicitly, set this parameter to true.
	 * 
	...
	/**
	 * Used to specify the name of a custom skin or template for the requested interstitial. Leaving it null will make
	 * the interstitial fall back to the DEFAULT template.
	 * 
	 * @param skinName
	 */
	...
	/**
	 * Sets the maximum amount of time the interstitial should take to load. If you set it to 0 or a negative number, it
	 * will fall back to the default value of 5 seconds.
	 * 
	 * @param loadingTimeoutSecs
	 */
	public void setLoadingTimeoutSecs(int loadingTimeoutSecs) {
		mLoadingTimeoutSecs = loadingTimeoutSecs > 0 ? loadingTimeoutSecs : LOADING_TIMEOUT_SECONDS_DEFAULT;
	}
	...
	public void startLoading() {
		...
		String[] interstitialUrlExtraValues = new String[] { URL_PARAM_INTERSTITIAL_VALUE_ON,
				URL_PARAM_ALLOW_CAMPAIGN_VALUE_ON, mSkinName, String.valueOf(sInterstitialAvailableResponseCount),
				mBackgroundUrl };
		...
		String interstitialUrl = UrlBuilder.buildUrl(interstitialBaseUrl, mUserId, mHostInfo, interstitialUrlExtraKeys,
	...
	/**
	 * Cancels the request and loading of the interstitial if the {@link InterstitialActivity} has still not been
	...
	/**
	 * Called when the results of the interstitial request are available, i.e. a response has been received from the
	 * server. Notifies the {@link InterstitialLoadingStatusListener} registered by the host publisher application of
	 * the events "Will Show Interstitial", "Interstitial Request Error" and "No Interstitial Available"
	 * 
	 * @param result
	 *            a {@link InterstitialLoadingResults} containing the status code and the contents of the response.
	 */
	...
	/**
	 * Launchs the {@link InterstitialActivity} with the initial contents of the interstitial ad, the initial base URL
	...
	private void launchInterstitialActivity(InterstitialLoadingResults result) {
		...
		interstitialIntent.putExtra(InterstitialActivity.EXTRA_INITIAL_CONTENT_KEY, result.returnedBody);
		interstitialIntent.putExtra(InterstitialActivity.EXTRA_COOKIESTRINGS_KEY, result.cookieStrings);
		interstitialIntent.putExtra(InterstitialActivity.EXTRA_SHOULD_STAY_OPEN_KEY, mShouldStayOpen);
